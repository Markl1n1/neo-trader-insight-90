...
const IntradayStrategy = ({ symbol, currentPrice, indicators }: IntradayStrategyProps) => {
  // Enhanced intraday strategy conditions based on flowchart
  const checkIntradayConditions = () => {
    ...
    // Step 1: Check Bollinger Bands (Price at Lower Band for LONG entry)
    const bollingerCondition = currentPrice <= indicators.bollingerLower * 1.005; // Within 0.5% of lower band
    
    // Step 2: Check MACD (MACD Line > Signal Line)
    const macdCondition = indicators.macd.line > indicators.macd.signal;
    
    // Step 3: Check RSI (RSI > 30 but < 70 - not oversold but not overbought)
    const rsiCondition = indicators.rsi > 30 && indicators.rsi < 70;
    
    // Step 4: Check Moving Averages (Price > 50 EMA for uptrend confirmation)
    const maCondition = currentPrice > indicators.ma50 && indicators.ma20 > indicators.ma50;
    
    // Step 5: Check Volume (above average volume)
    const volumeCondition = indicators.volume > indicators.avgVolume * 1.2;
    
    // Step 6: Check CVD (bullish trend for buying pressure)
      ...
      <div className="p-4 neo-border rounded-lg bg-card/50">
        ...
        <div className="grid grid-cols-1 gap-3 mb-4">
          ...
          <div className={`p-3 rounded-lg ${strategy.conditions.rsi ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
            <div className="flex justify-between items-center">
              <span className="font-medium">RSI (30-70)</span>
          ...
          <div className={`p-3 rounded-lg ${strategy.conditions.movingAverages ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
            <div className="flex justify-between items-center">
              <span className="font-medium">Above 50 EMA & 20 &gt; 50</span>
              <span className="font-mono">{currentPrice.toFixed(4)} / {indicators.ma50.toFixed(4)}</span>
              <span className={`px-2 py-1 rounded text-xs font-medium ${
          ...
          <div className={`p-3 rounded-lg ${strategy.conditions.volume ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
            <div className="flex justify-between items-center">
              <span className="font-medium">Above Avg Volume (1.2x)</span>
